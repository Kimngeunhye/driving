...
def pid_angle(ITerm, error, b_angle, b_error, Cnt):
    angle = 0
    Kp = 0.5  # 0.5 good / if Kp high -> loss decrease+faster respone but incur overshoot
    Ki = 0.0001 # 0.0001 good #0.0002 / if Ki high
    # -> accumulated loss increase faster+faster response but incur overshoot
    Kd = 1.0  # 1.0 good #2.0/ decrease the vibration
...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        if (ultra_msg[2] < 75 or ultra_msg[3] < 60) and Opt == 1:
            Opt = 2
            print(f'Opt :{Opt}, left :{ultra_msg[3]}, center: {ultra_msg[2]}')
